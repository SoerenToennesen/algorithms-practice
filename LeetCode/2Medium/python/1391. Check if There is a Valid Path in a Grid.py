...
#for some reason, the below code works better, even though I feel there is some problems with having to know which direction you are going.
'''
class Solution:
    def hasValidPath(self, grid: list[list[int]]) -> bool:
        if len(grid) == 1 and len(grid[0]) == 1:
        ...
        while True:
            ...
            for direct in directions:
                ...
                if not (test_tile[0] >= 0 and test_tile[0] < n and test_tile[1] >= 0 and test_tile[1] < m):
                    continue
                tile_test_type = grid_visited[test_tile[0]][test_tile[1]][0]
                if (not grid_visited[test_tile[0]][test_tile[1]][1]) and tile_test_type in directions_tiles[tile_type]:
                    if test_tile[0] == n - 1 and test_tile[1] == m - 1:
                        res = True
                        break
                    current = test_tile
